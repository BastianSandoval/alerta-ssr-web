...
@Component({
  selector: 'app-form-edit-validator',
  templateUrl: './form-edit-validator.component.html',
  styleUrls: ['./form-edit-validator.component.css']
})
export class FormEditValidatorComponent implements OnInit {
  ...
  public saveReport(event: Event, reportForm: FormGroupDirective) {
    ...
    if (this.checkoutForm.controls['name'].value != "" && this.checkoutForm.controls['email'].value != "")
  ...
  public async addInstitution(form: FormGroup, institution: FormGroupDirective): Promise<void> {
    try {
      ...
      await this.validatorProviderService.postCreateValidatorRequest(
        form.controls['name'].value,
        form.controls['lastName'].value,
        form.controls['email'].value,
        form.controls['rut'].value,
        this.institution,
      ).toPromise();
      this.checkoutForm.reset();
      institution.resetForm();
  ...
  public async setReport(): Promise<void> {
    ...
    try {
      ...
      // });
      // console.log(this.institution.atentionHour.slice(8, 13));
      // this.userFormControl.setValue([this.institution]);
      // this.categoryFormControl.setValue(this.institution.categories);
      // this.communesFormControl.setValue(this.institution.assignedCommunes);
      // if (this.institution.address != null)
      //   this.communeFormControl.setValue([this.institution]);
