...
export class GlobalMapComponent implements OnInit {
  ...
  async setReports(dataCategory?: any) {
    ...
    //   .toPromise();
    data = await this.reportProviderService
      .getAllReports(1, 100, this.withReview, this.rejected)
      .toPromise();
    // }
  ...
  async categoryFilter(event: any) {
    ...
    if (event.target.value != '') {
      ...
      // const data: any = await this.reportProviderService
      //   .getComplaintsPerCategory(this.filterCategory)
      //   .toPromise();
      // console.log(data);
      // if (data.docs.length === 0) {
